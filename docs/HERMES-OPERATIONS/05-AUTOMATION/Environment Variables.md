...
| `RENDER_DEPLOY_HOOK` | Private URL to trigger a Render deploy of `barrel-proof-flask` | `/opt/data/scripts/trigger_render_deploy.sh` (server-only) | Hostinger `/opt/data/.env` only — never commit |
